...
<div class="container" align="center">
						...
						<td title="${board.content }"> <!-- title=> 게시글 내용 미리보기 -->
							...
							<c:if test="${board.re_level > 0 }">
								...
								<a class="btn btn-info btn-sm"
									href="${path }/view/num/${board.num }/pageNum/${pb.currentPage }">${board.subject }</a>
									<%-- view/${board.num }/${pb.currentPage } 이렇게 써도 된다 --%>
							<!-- 조회수 50이 넘으면 인기글 -->
		...
		<ul class="pagination">
			...
			<c:forEach var="i" begin="${pb.startPage }" end="${pb.endPage }">
				...
				<c:if test="${pb.currentPage == i }">
					<li class="active"><a href="${path }/list/pageNum/${i }">${i }</a></li>
				</c:if>
				<c:if test="${pb.currentPage != i }">
			...
			<c:if test="${pb.endPage < pb.totalPage }">
				...
				<li>
					<a href="${path }/list/pageNum/${pb.totalPage }?search=${board.search }&keyword=${board.keyword }">	<!-- 끝페이지로 바로 이등(순서 생각하며 넣기) -->
	...
	<form action="${path }/list/pageNum/1">
		<!-- <select name="search">	model에 추가해줘야한다
			<option value="writer">작성자</option>
			<option value="subject">제목</option>
			<option value="content">내용</option>
		</select> -->
		<select name="search">
			<c:forTokens var="sh" items="writer,subject,content,subcon" delims="," varStatus="i">	<!-- items내에 띄어쓰면 안된다! -->
		...
		<input type="text" name="keyword" value="${board.keyword }">	<!-- model에 추가해줘야한다 -->
